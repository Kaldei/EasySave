...
namespace EasySave.NS_Model
{
    public class Work
    {
        ...
        // --- Methods ----
        // Save Log 
        public void SaveLog(DateTime _startTime, string _src, string _dst, long _size, int _elapsedTime)
        {
            // Prepare times log
            ...
            logs.Add(new Log($"{this.name}", $"{_src}", $"{_dst}", $"{_size}", $"{_startTime}", $"{_elapsedTime}"));
